...
- **属性注入**：`<meta name="description" data-i18n-content="key">`（写成 content 属性，不是文本）

### 文案的三层结构（改之前先搞清楚写哪一层）

| 层 | 位置 | 作用 |
|---|---|---|
| 1 | HTML 里的静态文本 | **无 JS 时的兜底**，也是 SEO 爬虫看到的 |
| 2 | `assets/js/i18n.js` | 全站默认文案（233 键 × 3 语言） |
| 3 | `content/site.json` 的 `copy` | 后台改过的**覆盖值**，只存被改过的键 |

`main.js` 启动时把第 3 层合并进第 2 层，再注入到所有 `data-i18n` 节点。

⚠️ **三层必须同步**：只改 i18n 不改 HTML，JS 加载失败时就会露出旧文案。
`_tools/verify.py` 有一条断言专门比对各页静态兜底与 i18n 的一致性，改完跑一次即可发现漂移。

页面 `<title>` 与 `<meta name="description">` 也走第 2/3 层，键为 `meta.title.<页>` / `meta.desc.<页>`
（页 = home / works / about / services / skills / contact / resume / work），切换语言时标签页标题会跟着变。
...
- 功能专项：首页精选 1 个、作品集 1 个、筛选各返回正确数量、图集 25 张、
  ...
  三语键完整性（233 键 × 3 无缺键无空值）、页面标题与描述随语言切换、
  关于我（优势 5 / 原则 5 节点 / 时间线 3）、服务范围（6 板块 + 序号）、技能体系（能力 8 / 工具 7）、
  联系方式（7 个合作类型选项）、**各页静态兜底文案与 i18n 零漂移**
- 移动端 390px 无横向溢出
...
| 12 | 其他页面文案 | 左侧「页面文案 · 关于 / 服务 / 技能 / 联系」→ 8 个页签，共 157 条 |
...
后台左侧**「页面文案 · 关于 / 服务 / 技能 / 联系」**，8 个页签覆盖除主页/页脚以外的全部界面文案：

| 页签 | 条数 | 内容 |
|---|---|---|
| 关于我 | 26 | 标题导语、设计原则、经历时间线（3 段）、能力优势（5 条） |
| 服务范围 | 22 | 页面头部 + 6 个服务板块（序号 / 标题 / 描述）+ 底部按钮 |
| 技能体系 | 21 | 页面头部 + 8 条专业能力 + 7 个工具 + 补充说明 |
| 联系方式 | 25 | 信息条（所在地 / 邮箱 / 微信 / 合作方式）、表单标签、7 个合作类型选项、提交提示 |
| 作品集列表 | 19 | 页面头部、8 个分类筛选按钮、7 个卡片标签 |
| 案例详情页 | 18 | 区块标题、元信息、图集计数与导航、兜底正文 |
| 页面标题与描述 | 16 | 8 个页面的 `<title>` 与 meta description |
| 导航与通用 | 10 | 全站导航 8 项 + 二维码弹层提示 |

**未改动的键不会写进 `site.json`**，界面直接显示 i18n 默认值；改过的行会出现「已改」徽标和「重置」按钮，
点重置即删掉覆盖值、恢复默认。保存走 `PUT /api/site`，与「站点设置」同一个接口。
